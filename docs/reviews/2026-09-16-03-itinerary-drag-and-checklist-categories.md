...
- 사용자 요청에 따라 구현 결과를 리뷰로 이관하고 플랜을 삭제한다. 아래의 실기기 VoiceOver 및 직접 마우스 재검증 미확인을 통과로 처리하지 않는다.
...
- 수정 후 사람의 직접 마우스 드래그 재검증은 회신을 받지 못했다. CUA·Orca의 마우스 입력은 순서 변경을 확인하지 못했으므로 성공 증거에 포함하지 않았다. 빠른 iOS 터치 드래그와 본문 스크롤은 아래 Maestro·SQLite 검증으로 확인했다.
