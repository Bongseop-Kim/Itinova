...
| 11 | `11-clay-assets-and-visual-refresh.md` | 클레이 3D 에셋 16종(GPT Image 2.5, 앵커 참조 편집, 선형 광 축소→WebP), 빈 상태·온보딩·S20 헤더·썸네일·S03·S12·아이콘 반영 | — | OpenAI API 키 |
| 12 | `12-designbase-component-refresh.md` | Designbase 템플릿(pen.dev에서 추출, `docs/references/designbase-app-ui/`) 기준 공용 컴포넌트 15종·아이콘 40여 종(SVG, expo-image) 도입, 라운드·시맨틱 토큰 개편, S13·S20·S01·S04 배치 변경 | — (11과 병행, 12 먼저) | 템플릿 라이선스 확인 |

권장 순서: **01 → (02, 03, 04 병행) → 05 → 06 → 07 → 08 → 09 → 10**. 05는 GCP 준비가 되는 즉시 01과 병행할 수 있다.
10의 C01·C04·콘텐츠 정리는 01 직후 언제든 끼워 넣을 수 있다. 11·12는 선행이 없어 언제든 단독 실행한다. 같은 파일을 만질 때는 12(구조) → 11(그림) 순서.
...
- [ ] 빈 상태·온보딩·S20·썸네일·S03·S12에 클레이 에셋 16종이 들어가고 `app.json`에 `#1554D1`이 없다(11).
- [ ] 헤더·시트·검색에 텍스트 액션이 없고 탭바에 아이콘이 있으며 화면별 `Field`·scrim 중복이 없다(12).
