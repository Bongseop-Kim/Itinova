...
코드 구현과 순수 로직 검사는 완료했으나 UI 검증이 남아 있어 이 플랜은 완료 처리하지 않는다.

- 구현 커밋: AI 이탈 `99a7575`, 기간 축소 확인 `2dcb7e0`, 기본 체크리스트 섹션 `d70b26f`, 비용 입력 `265fd90`, 순수 검사 `0b332c2`.
- `npm run check`, `npx tsc --noEmit`, `git diff --check` 통과. `/trip/1/` 링크 0건.
- CUA로 S07 뒤로 → 생성 방식 시트, S08 홈으로 → 홈 복귀 확인.
- 기간 축소 확인창에 장소 2개 표시 및 확인 전 DB의 일차 3개·일정 장소 2개·저장 장소 1개 유지 확인. 취소 버튼의 동작을 검증한 것은 아니다.
- simctl 딥링크·스크린샷으로 THB 최초 선택, `day=99`의 미지정 처리, 연결 장소명 표시 확인.
- 남은 검증: 기간 축소 취소/확인 및 증가 후 DB 비교, 체크리스트 삭제/재추가 및 정렬, BS1 비용 진입·KRW 변경 유지·저장 결과, 다른 여행 장소 저장 차단, AI 수동 생성 버튼·채팅 뒤로가기.
- 차단 원인: `cua_repl`이 `Sky Computer Use native pipe startup failed`로 반복 실패하며 세션 초기화 후에도 동일. Maestro 미설치. 사용자 지시에 따라 Orca 대체 조작 중단.
- 확인창은 앱 재실행으로 닫았으며 기간 변경을 적용하지 않았다. 검증 전용 여행과 장소만 정리하며 기존 여행은 보존한다.
- 구현 차이: 체크리스트 섹션 순수 함수는 기존 템플릿 파일 `db/templates.ts`에 두어 새 파일을 만들지 않았다. 일차는 비동기 조회 후 범위 검증을 적용하고 사용자의 미지정 선택은 별도로 유지한다.
