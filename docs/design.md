...
- 공개 엔드포인트는 `POST /v1/itineraries/generate`, `POST /v1/chat` 두 개로 시작한다.
...
- 계정 없는 공개 앱이므로 Firebase App Check(App Attest/Play Integrity) 토큰을 서버에서 검증하고 요청 빈도 제한, 최대 인스턴스 제한, 예산 알림을 적용해 무단 호출과 비용 폭증을 막는다.
...
| 항목 | 현재 결정 | 미결/후속 |
| --- | --- | --- |
| **AI 서버와 API 키 관리** | GCP Cloud Run `itinova-api` + Google Secret Manager | ⚠️ **배포 차단 항목.** Claude 키를 앱 번들·Expo 환경 변수·로그에 넣지 않는다. 런타임 서비스 계정에 Secret Manager 읽기 권한만 부여한다 |
...
| S07·S08·S19 AI | 라우트 골격 | Cloud Run `itinova-api`, Secret Manager, Apple 검색을 통한 장소 해석, 초안 검증·저장·채팅 |
