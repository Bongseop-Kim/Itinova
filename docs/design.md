...
3. **iOS 장소 검색은 Apple MapKit으로 전환**했다. 별도 검색 키나 프록시는 필요 없다. Claude 연동은 서버 프록시 뒤에서 구현하며 Android 지도 키는 별도 관리한다 → §6.
...
| 번역 | 입력/대상 언어 선택 후 외부 Google 번역에서 열기 | 앱 내부 번역은 별도 API 또는 Claude 프록시 필요 |
...
| S12 여행 도구 | Open-Meteo 날씨, 한국/현지 시각, Frankfurter 환율, 외부 Google 번역 연결 | 앱 내부 번역 응답은 번역 API 또는 Claude 프록시 필요 |
...
| S07·S08·S19 AI | 라우트 골격 | Claude 키·프록시, Apple 검색을 통한 장소 해석, 초안 검증·저장·채팅 |
